...
final class MovieQuizViewController: UIViewController, QuestionFactoryDelegate {
    ...
    private func showResults() {
        ...
        let gamePercentage = Int((Double(correctAnswers) / Double(questionsAmount)) * 100)
        let bestGameDate = bestGame.date
        
        let alertModel = AlertModel(
            title: "Раунд окончен!",
            message: """
    Ваш результат: \(correctAnswers) из \(questionsAmount) (\(gamePercentage)%).
    Всего игр сыграно: \(totalGames).
    Ваш лучший результат: \(bestGame.correct) из \(bestGame.total) (\(bestGame.total)%).
    Дата лучшего результата: \(bestGameDate).
    """,
